...
- The IKFast tree was originally vendored at `src/ssik/_vendor/` during the early port-and-patch phase; it has been removed (PR resolving [#84](https://github.com/siddhss5/ikfastpy/issues/84)). The decision to remove rather than preserve was driven by the conformance evidence: the new pipeline covers every case the vendored solver was needed for, at machine precision, faster, on every CI matrix entry. The vendored tree contributed nothing the rewrite couldn't do better.
