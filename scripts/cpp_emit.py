...
def emit(arm: str, out_dir: Path, n_parity: int = 200, seed: int = 0) -> None:
    ...
    if _is_three_parallel(kb):
        # Solve-level conformance is covered by the reused Python suite (the C++
        # backend runs tests/test_three_parallel.py via the pybind binding, #499),
        # so no solve-parity golden is emitted. Only the Newton primitive keeps a
        # Python-free golden (its refinement path is a no-op on the reused suite's
        # happy-path poses, so it needs its own targeted fixture).
