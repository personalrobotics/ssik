"""Bulletproof validation for the cr5_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.cr5_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm cr5_ik`` first):
...
``[arms.cr5_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/cr5_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "cr5_ik.urdf"
...
def test_cr5_ik_loads_as_6dof() -> None:
    kb = _cr5_ik_kinbody()
...
def test_cr5_ik_dispatches_to_ikgeo_three_parallel() -> None:
...
def test_cr5_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.cr5_ik",
        reason="build the artifact first: regen_artifacts.py --arm cr5_ik",
    )
...
def test_cr5_ik_random_pose_fk_closure(seed: int) -> None:
