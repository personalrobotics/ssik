"""Bulletproof validation for the fanuc_m710ic_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.fanuc_m710ic_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm fanuc_m710ic_ik`` first):
...
``[arms.fanuc_m710ic_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/fanuc_m710ic_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "fanuc_m710ic_ik.urdf"
...
def test_fanuc_m710ic_ik_loads_as_6dof() -> None:
    kb = _fanuc_m710ic_ik_kinbody()
...
def test_fanuc_m710ic_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_fanuc_m710ic_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.fanuc_m710ic_ik",
        reason="build the artifact first: regen_artifacts.py --arm fanuc_m710ic_ik",
    )
...
def test_fanuc_m710ic_ik_random_pose_fk_closure(seed: int) -> None:
