"""Bulletproof validation for the gen72_ik fixture (auto-generated by ``ssik add-arm``).

The arm dispatches to ``seven_r.spherical_shoulder_polished`` (tier 0) per the
current ``ssik.core.dispatcher``. The scaffold below verifies, against the
**shipped artifact** (``ssik.prebuilt.gen72_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm gen72_ik`` first):
...
``[arms.gen72_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/gen72_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "gen72_ik.urdf"
...
def test_gen72_ik_loads_as_7dof() -> None:
    kb = _gen72_ik_kinbody()
...
def test_gen72_ik_dispatches_to_seven_r_spherical_shoulder_polished() -> None:
    """Dispatcher routing is stable. Updating the dispatcher should
    update this assertion deliberately.
    """
    kb = _gen72_ik_kinbody()
    plan = dispatch(kb)
    assert plan.solver_name == "seven_r.spherical_shoulder_polished"
    assert plan.tier == 0
...
def test_gen72_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.gen72_ik",
        reason="build the artifact first: regen_artifacts.py --arm gen72_ik",
    )
...
def test_gen72_ik_random_pose_fk_closure(seed: int) -> None:
