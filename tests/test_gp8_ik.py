"""Bulletproof validation for the gp8_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.gp8_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm gp8_ik`` first):
...
``[arms.gp8_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/gp8_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "gp8_ik.urdf"
...
def _gp8_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "base_link", "link_6_t")
...
def test_gp8_ik_loads_as_6dof() -> None:
    kb = _gp8_ik_kinbody()
...
def test_gp8_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_gp8_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.gp8_ik",
        reason="build the artifact first: regen_artifacts.py --arm gp8_ik",
    )
...
def test_gp8_ik_random_pose_fk_closure(seed: int) -> None:
