"""Bulletproof validation for the hc10_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.hc10_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm hc10_ik`` first):
...
``[arms.hc10_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/hc10_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "hc10_ik.urdf"
...
def test_hc10_ik_loads_as_6dof() -> None:
    kb = _hc10_ik_kinbody()
...
def test_hc10_ik_dispatches_to_ikgeo_general_6r() -> None:
...
def test_hc10_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.hc10_ik",
        reason="build the artifact first: regen_artifacts.py --arm hc10_ik",
    )
...
def test_hc10_ik_random_pose_fk_closure(seed: int) -> None:
