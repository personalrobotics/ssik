"""Bulletproof validation for the hh020_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.hh020_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm hh020_ik`` first):
...
``[arms.hh020_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/hh020_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "hh020_ik.urdf"
...
def test_hh020_ik_loads_as_6dof() -> None:
    kb = _hh020_ik_kinbody()
...
def test_hh020_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_hh020_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.hh020_ik",
        reason="build the artifact first: regen_artifacts.py --arm hh020_ik",
    )
...
def test_hh020_ik_random_pose_fk_closure(seed: int) -> None:
