"""Bulletproof validation for the irb120_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.irb120_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm irb120_ik`` first):
...
``[arms.irb120_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/irb120_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "irb120_ik.urdf"
...
def test_irb120_ik_loads_as_6dof() -> None:
    kb = _irb120_ik_kinbody()
...
def test_irb120_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_irb120_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.irb120_ik",
        reason="build the artifact first: regen_artifacts.py --arm irb120_ik",
    )
...
def test_irb120_ik_random_pose_fk_closure(seed: int) -> None:
