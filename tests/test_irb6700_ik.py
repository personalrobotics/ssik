"""Bulletproof validation for the irb6700_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.irb6700_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm irb6700_ik`` first):
...
``[arms.irb6700_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/irb6700_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "irb6700_ik.urdf"
...
def test_irb6700_ik_loads_as_6dof() -> None:
    kb = _irb6700_ik_kinbody()
...
def test_irb6700_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_irb6700_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.irb6700_ik",
        reason="build the artifact first: regen_artifacts.py --arm irb6700_ik",
    )
...
def test_irb6700_ik_random_pose_fk_closure(seed: int) -> None:
