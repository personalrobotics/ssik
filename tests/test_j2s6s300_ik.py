"""Bulletproof validation for the j2s6s300_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.j2s6s300_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm j2s6s300_ik`` first):
...
``[arms.j2s6s300_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/j2s6s300_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "j2s6s300_ik.urdf"
...
def _j2s6s300_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "j2s6s300_link_base", "j2s6s300_end_effector")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _j2s6s300_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_j2s6s300_ik_loads_as_6dof() -> None:
    kb = _j2s6s300_ik_kinbody()
...
def test_j2s6s300_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_j2s6s300_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.j2s6s300_ik",
        reason="build the artifact first: regen_artifacts.py --arm j2s6s300_ik",
    )
...
def test_j2s6s300_ik_random_pose_fk_closure(seed: int) -> None:
