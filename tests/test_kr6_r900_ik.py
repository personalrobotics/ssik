"""Bulletproof validation for the kr6_r900_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.kr6_r900_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm kr6_r900_ik`` first):
...
``[arms.kr6_r900_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/kr6_r900_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "kr6_r900_ik.urdf"
...
def test_kr6_r900_ik_loads_as_6dof() -> None:
    kb = _kr6_r900_ik_kinbody()
...
def test_kr6_r900_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_kr6_r900_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.kr6_r900_ik",
        reason="build the artifact first: regen_artifacts.py --arm kr6_r900_ik",
    )
...
def test_kr6_r900_ik_random_pose_fk_closure(seed: int) -> None:
