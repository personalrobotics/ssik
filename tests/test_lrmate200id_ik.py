"""Bulletproof validation for the lrmate200id_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.lrmate200id_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm lrmate200id_ik`` first):
...
``[arms.lrmate200id_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/lrmate200id_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "lrmate200id_ik.urdf"
...
def test_lrmate200id_ik_loads_as_6dof() -> None:
    kb = _lrmate200id_ik_kinbody()
...
def test_lrmate200id_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_lrmate200id_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.lrmate200id_ik",
        reason="build the artifact first: regen_artifacts.py --arm lrmate200id_ik",
    )
...
def test_lrmate200id_ik_random_pose_fk_closure(seed: int) -> None:
