"""Bulletproof validation for the m0609_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.m0609_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm m0609_ik`` first):
...
``[arms.m0609_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/m0609_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "m0609_ik.urdf"
...
def test_m0609_ik_loads_as_6dof() -> None:
    kb = _m0609_ik_kinbody()
...
def test_m0609_ik_dispatches_to_ikgeo_general_6r() -> None:
...
def test_m0609_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.m0609_ik",
        reason="build the artifact first: regen_artifacts.py --arm m0609_ik",
    )
...
def test_m0609_ik_random_pose_fk_closure(seed: int) -> None:
