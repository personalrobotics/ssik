"""Bulletproof validation for the nova5_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.nova5_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm nova5_ik`` first):
...
``[arms.nova5_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/nova5_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "nova5_ik.urdf"
...
def test_nova5_ik_loads_as_6dof() -> None:
    kb = _nova5_ik_kinbody()
...
def test_nova5_ik_dispatches_to_ikgeo_three_parallel() -> None:
...
def test_nova5_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.nova5_ik",
        reason="build the artifact first: regen_artifacts.py --arm nova5_ik",
    )
...
def test_nova5_ik_random_pose_fk_closure(seed: int) -> None:
