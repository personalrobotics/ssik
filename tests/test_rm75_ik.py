"""Bulletproof validation for the rm75_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.rm75_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm rm75_ik`` first):
...
``[arms.rm75_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/rm75_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "rm75_ik.urdf"
...
def _rm75_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "base_link", "link_7")
...
def test_rm75_ik_loads_as_7dof() -> None:
    kb = _rm75_ik_kinbody()
...
def test_rm75_ik_dispatches_to_seven_r_srs_polished() -> None:
...
def test_rm75_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.rm75_ik",
        reason="build the artifact first: regen_artifacts.py --arm rm75_ik",
    )
...
def test_rm75_ik_random_pose_fk_closure(seed: int) -> None:
