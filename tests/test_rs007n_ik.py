"""Bulletproof validation for the rs007n_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.rs007n_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm rs007n_ik`` first):
...
``[arms.rs007n_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/rs007n_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "rs007n_ik.urdf"
...
def test_rs007n_ik_loads_as_6dof() -> None:
    kb = _rs007n_ik_kinbody()
...
def test_rs007n_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_rs007n_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.rs007n_ik",
        reason="build the artifact first: regen_artifacts.py --arm rs007n_ik",
    )
...
def test_rs007n_ik_random_pose_fk_closure(seed: int) -> None:
