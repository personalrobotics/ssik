"""Bulletproof validation for the rv4fr_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.rv4fr_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm rv4fr_ik`` first):
...
``[arms.rv4fr_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/rv4fr_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "rv4fr_ik.urdf"
...
def _rv4fr_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "rv4fr_base", "rv4fr_hand_flange")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _rv4fr_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_rv4fr_ik_loads_as_6dof() -> None:
    kb = _rv4fr_ik_kinbody()
...
def test_rv4fr_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_rv4fr_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.rv4fr_ik",
        reason="build the artifact first: regen_artifacts.py --arm rv4fr_ik",
    )
...
def test_rv4fr_ik_random_pose_fk_closure(seed: int) -> None:
