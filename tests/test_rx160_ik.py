"""Bulletproof validation for the rx160_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.rx160_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm rx160_ik`` first):
...
``[arms.rx160_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/rx160_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "rx160_ik.urdf"
...
def test_rx160_ik_loads_as_6dof() -> None:
    kb = _rx160_ik_kinbody()
...
def test_rx160_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_rx160_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.rx160_ik",
        reason="build the artifact first: regen_artifacts.py --arm rx160_ik",
    )
...
def test_rx160_ik_random_pose_fk_closure(seed: int) -> None:
