"""Bulletproof validation for the viperx300s_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.viperx300s_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm viperx300s_ik`` first):
...
``[arms.viperx300s_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source MuJoCo MJCF: ``tests/fixtures/viperx300s_ik.xml`` (vendored via ``ssik add-arm``).
"""
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "viperx300s_ik.xml"
...
def _viperx300s_ik_kinbody():
    return load_mjcf_kinbody_normalized(FIXTURE_PATH, "base_link", "gripper_link")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _viperx300s_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_viperx300s_ik_loads_as_6dof() -> None:
    kb = _viperx300s_ik_kinbody()
...
def test_viperx300s_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_viperx300s_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.viperx300s_ik",
        reason="build the artifact first: regen_artifacts.py --arm viperx300s_ik",
    )
...
def test_viperx300s_ik_random_pose_fk_closure(seed: int) -> None:
