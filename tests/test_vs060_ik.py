"""Bulletproof validation for the vs060_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.vs060_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm vs060_ik`` first):
...
``[arms.vs060_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/vs060_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "vs060_ik.urdf"
...
def test_vs060_ik_loads_as_6dof() -> None:
    kb = _vs060_ik_kinbody()
...
def test_vs060_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
...
def test_vs060_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.vs060_ik",
        reason="build the artifact first: regen_artifacts.py --arm vs060_ik",
    )
...
def test_vs060_ik_random_pose_fk_closure(seed: int) -> None:
