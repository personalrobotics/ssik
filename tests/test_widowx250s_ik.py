"""Bulletproof validation for the widowx250s_ik fixture (auto-generated by ``ssik add-arm``).

The arm dispatches to ``ikgeo.spherical_two_parallel`` (tier 0) per the
current ``ssik.core.dispatcher``. The scaffold below verifies, against the
**shipped artifact** (``ssik.prebuilt.widowx250s_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm widowx250s_ik`` first):

- MuJoCo MJCF loads as a 6-DOF chain (revolute / continuous joints).
...
``[arms.widowx250s_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source MuJoCo MJCF: ``tests/fixtures/widowx250s_ik.xml`` (vendored via ``ssik add-arm``).
"""
...
from ssik._mjcf import load_mjcf_kinbody_normalized
from ssik.core.dispatcher import dispatch

FIXTURE_PATH = Path(__file__).parent / "fixtures" / "widowx250s_ik.xml"
...
def _widowx250s_ik_kinbody():
    return load_mjcf_kinbody_normalized(FIXTURE_PATH, "wx250s/base_link", "wx250s/gripper_link")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _widowx250s_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_widowx250s_ik_loads_as_6dof() -> None:
    kb = _widowx250s_ik_kinbody()
...
def test_widowx250s_ik_dispatches_to_ikgeo_spherical_two_parallel() -> None:
    """Dispatcher routing is stable. Updating the dispatcher should
    update this assertion deliberately.
    """
    kb = _widowx250s_ik_kinbody()
    plan = dispatch(kb)
    assert plan.solver_name == "ikgeo.spherical_two_parallel"
    assert plan.tier == 0
...
def test_widowx250s_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.widowx250s_ik",
        reason="build the artifact first: regen_artifacts.py --arm widowx250s_ik",
    )
...
def test_widowx250s_ik_random_pose_fk_closure(seed: int) -> None:
