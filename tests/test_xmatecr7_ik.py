"""Bulletproof validation for the xmatecr7_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.xmatecr7_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm xmatecr7_ik`` first):
...
``[arms.xmatecr7_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/xmatecr7_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "xmatecr7_ik.urdf"
...
def _xmatecr7_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "xMateCR7_base", "xMateCR7_link6")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _xmatecr7_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_xmatecr7_ik_loads_as_6dof() -> None:
    kb = _xmatecr7_ik_kinbody()
...
def test_xmatecr7_ik_dispatches_to_ikgeo_general_6r() -> None:
...
def test_xmatecr7_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.xmatecr7_ik",
        reason="build the artifact first: regen_artifacts.py --arm xmatecr7_ik",
    )
...
def test_xmatecr7_ik_random_pose_fk_closure(seed: int) -> None:
