"""Bulletproof validation for the xmatepro7_ik fixture (auto-generated by ``ssik add-arm``).

The arm dispatches to ``seven_r.srs`` (tier 0) per the
current ``ssik.core.dispatcher``. The scaffold below verifies, against the
**shipped artifact** (``ssik.prebuilt.xmatepro7_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm xmatepro7_ik`` first):
...
``[arms.xmatepro7_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/xmatepro7_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "xmatepro7_ik.urdf"
...
def _xmatepro7_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "xMatePro7_base", "xMatePro7_link7")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _xmatepro7_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_xmatepro7_ik_loads_as_7dof() -> None:
    kb = _xmatepro7_ik_kinbody()
...
def test_xmatepro7_ik_dispatches_to_seven_r_srs() -> None:
...
def test_xmatepro7_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.xmatepro7_ik",
        reason="build the artifact first: regen_artifacts.py --arm xmatepro7_ik",
    )
...
def test_xmatepro7_ik_random_pose_fk_closure(seed: int) -> None:
