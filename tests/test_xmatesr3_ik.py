"""Bulletproof validation for the xmatesr3_ik fixture (auto-generated by ``ssik add-arm``).
...
**shipped artifact** (``ssik.prebuilt.xmatesr3_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm xmatesr3_ik`` first):
...
``[arms.xmatesr3_ik.known_gaps]`` entry in MANIFEST.toml documenting why.

Source URDF: ``tests/fixtures/xmatesr3_ik.urdf`` (vendored via ``ssik add-arm``).
"""
...
URDF_PATH = Path(__file__).parent / "fixtures" / "xmatesr3_ik.urdf"
...
def _xmatesr3_ik_kinbody():
    return load_urdf_kinbody_normalized(URDF_PATH, "xMateSR3_base", "xMateSR3_link6")


def _joint_bounds():
    """(lo, hi) arrays of per-joint sampling bounds from the real URDF
    limits; continuous / limitless joints sample [-pi, pi]."""
    kb = _xmatesr3_ik_kinbody()
    lo, hi = [], []
    for j in kb.joints:
...
def test_xmatesr3_ik_loads_as_6dof() -> None:
    kb = _xmatesr3_ik_kinbody()
...
def test_xmatesr3_ik_dispatches_to_ikgeo_general_6r() -> None:
...
def test_xmatesr3_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.xmatesr3_ik",
        reason="build the artifact first: regen_artifacts.py --arm xmatesr3_ik",
    )
...
def test_xmatesr3_ik_random_pose_fk_closure(seed: int) -> None:
