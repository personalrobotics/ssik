...
**shipped artifact** (``ssik.prebuilt.yumi_left_ik`` -- build it with
``uv run python scripts/regen_artifacts.py --arm yumi_left_ik`` first):
...
``[arms.yumi_left_ik.known_gaps]`` entry in MANIFEST.toml documenting why.
...
def test_yumi_left_ik_loads_as_7dof() -> None:
    kb = _yumi_left_ik_kinbody()
...
def test_yumi_left_ik_coverage_and_fk_closure() -> None:
    ...
    art = pytest.importorskip(
        "ssik.prebuilt.yumi_left_ik",
        reason="build the artifact first: regen_artifacts.py --arm yumi_left_ik",
    )
    ...
    assert coverage >= _MIN_COVERAGE, (
        f"yumi_left_ik: coverage {coverage:.0%} < {_MIN_COVERAGE:.0%} "
    ...
    assert worst_fk < _FK_CEILING, (
        f"yumi_left_ik: worst FK {worst_fk:.2e} > {_FK_CEILING:.0e} ceiling"
    )
